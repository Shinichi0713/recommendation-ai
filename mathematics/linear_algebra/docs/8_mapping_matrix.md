...
3次元空間にある点（ベクトル ）に、真上から光を当てて、床（2次元の平面）に影を落とす様子を想像してください。
...
行列  が射影行列であるためには、次の**決定的な性質**を満たす必要があります。
...
特定のベクトル  方向（1次元の線）に影を落とす射影行列  は、以下の式で作ることができます。

* **分母 :** ベクトルの長さの2乗（正規化のための調整）。
* **分子 :** 「列ベクトル × 行ベクトル」で作られる行列（外積）。
