...
SVD（特異値分解）の数値計算アルゴリズムは、**「QR分解 → 二重対角化 → 二重対角行列のSVD → 逆変換」** という流れで構成されることが多いです。  
以下、その概要をステップごとに説明します。

### 1. 全体の流れ（高レベル）

任意の $ m \times n $ 行列 $ A $ のSVDを計算する標準的な手順は、概ね次の4ステップです：

1. **QR分解**：$ A $ を直交行列 $ Q $ と上三角行列 $ R $ に分解し、問題を「細長い行列」に縮小
2. **二重対角化（bidiagonalization）**：$ R $（または $ A $）を二重対角行列 $ B $ に変換（左右から直交変換）
3. **二重対角行列のSVD**：$ B $ のSVDを計算（高速・安定）
4. **逆変換**：得られた特異値・特異ベクトルを元の $ A $ の基底に戻す

このアプローチは、Golub–Kahan型のアルゴリズムとして知られています。

### 2. ステップ1：QR分解

__目的__
- $ m \ge n $ のとき、$ A $ を「細長い」 $ n \times n $ の上三角部分に縮小する
- これにより、以降の計算コストを削減

__手順__
1. $ A \in \mathbb{R}^{m \times n} $ に対し、Householder変換やGivens回転を用いてQR分解：
   $$
   A = Q R
   $$
   - $ Q \in \mathbb{R}^{m \times m} $：直交行列
   - $ R \in \mathbb{R}^{m \times n} $：上三角部分が非ゼロ（下はゼロ）

2. $ m > n $ の場合、$ R $ の上 $ n \times n $ 部分だけを残し、残りを無視：
   $$
   R = \begin{bmatrix} R_1 \\ 0 \end{bmatrix}, \quad R_1 \in \mathbb{R}^{n \times n}
   $$
   これにより、問題は $ n \times n $ 行列 $ R_1 $ のSVDに帰着。

**ポイント**：QR分解により、行列の「高さ」を削減し、正方行列の問題に帰着させます。

### 3. ステップ2：二重対角化（Bidiagonalization）

__目的__
- 正方行列 $ R_1 $（または $ A $）を、**二重対角行列** $ B $ に変換：
  $$
  B = \begin{bmatrix}
  d_1 & e_1 & & \\
      & d_2 & e_2 & \\
...
__手順（Golub–Kahanの二重対角化）__

1. 左からHouseholder変換 $ U_k $ を適用し、列の下部分をゼロにする（QR分解の要領）
2. 右からHouseholder変換 $ V_k $ を適用し、行の右部分をゼロにする
3. これを繰り返すと、最終的に
   $$
   U^\top A V = B
   $$
   となる直交行列 $ U, V $ と二重対角行列 $ B $ が得られる

**ポイント**：
- 二重対角行列は、**非ゼロ要素が対角とその隣のみ**という非常に疎な構造
- この形にすると、次のSVD計算が大幅に高速化・安定化される
...
- 二重対角行列 $ B $ のSVDを計算：
  $$
  B = \tilde{U} \Sigma \tilde{V}^\top
  $$

__代表的なアルゴリズム：分割征服法（Divide-and-Conquer）__

1. **対称三重対角行列への変換**：
   - $ B^\top B $ や $ B B^\top $ は対称三重対角行列になる
   - あるいは、$ B $ を直接扱うアルゴリズムも存在

2. **分割**：
   - 行列をブロックに分割し、それぞれのSVD（または固有値問題）を再帰的に解く

3. **統合**：
   - 部分解を組み合わせて、全体の特異値・特異ベクトルを構成
   - この過程で、**高速な直交変換（Givens回転など）**を用いる

**ポイント**：
- 二重対角（または三重対角）という疎構造を活かし、$ O(n^2) $ 程度のコストでSVDを計算可能
- 一般の密行列のSVD（$ O(mn^2) $）に比べて効率的

### 5. ステップ4：逆変換

__目的__
- 二重対角行列 $ B $ のSVDから、元の行列 $ A $ のSVDを復元する
...
3. 代入：
   $$
   A = (U \tilde{U}) \Sigma (\tilde{V}^\top V^\top) = U_\text{final} \Sigma V_\text{final}^\top
   $$
   ここで：
   - $ U_\text{final} = U \tilde{U} $
   - $ V_\text{final} = V \tilde{V} $

**ポイント**：
- 直交行列の積は直交行列なので、$ U_\text{final}, V_\text{final} $ も直交
- これにより、元の $ A $ のSVDが得られる

### 6. アルゴリズムの特徴・利点

__数値的安定性__
- Householder変換やGivens回転は**数値的に安定**な直交変換
- 二重対角化により、問題を「疎な構造」に落とし込むため、誤差の蓄積が抑制される

__計算コスト__
- QR分解：$ O(mn^2) $
- 二重対角化：$ O(mn^2) $
- 二重対角行列のSVD：$ O(n^2) $（分割征服法など）
- 全体：$ O(mn^2) $（$ m \ge n $ のとき）

一般の密行列のSVDは、このオーダーが下限とされています。

__実装__
- LAPACKの `dgesvd` ルーチンなどが、この流れに基づく実装
- Pythonの `numpy.linalg.svd` も、内部でLAPACKを呼び出していることが多い

### 7. パッケージ

PythonでSVDを実装している主なパッケージと、その注意点を簡潔にまとめます。

__1. 代表的なパッケージ__

__NumPy__
- **`numpy.linalg.svd`**  
  - LAPACK（`dgesvd` / `zgesvd` など）を呼び出す薄いラッパー  
  - QR＋二重対角化＋分割征服SVDの流れに基づく実装  
  - 密行列用で、実用上最も一般的

__SciPy__
- **`scipy.linalg.svd`**  
  - 内部実装はNumPyとほぼ同じ（LAPACK利用）  
  - 追加で `lapack_driver` オプションがあり、使用するLAPACKルーチンを選択可能  
    - `'gesdd'`：分割征服法（divide-and-conquer、高速・メモリ多め）  
    - `'gesvd'`：標準的なSVD（やや遅いが安定）

__Scikit-learn__
- **`sklearn.utils.extmath.randomized_svd`**  
  - 大規模行列向けの**乱択SVD（randomized SVD）**  
  - 低ランク近似に特化して高速化  
  - 完全なSVDではなく、近似解を返す

__PyTorch / JAX__
- **`torch.linalg.svd`**, **`jax.numpy.linalg.svd`**  
  - GPU対応・自動微分対応のSVD  
  - バックエンドはCUDA/cuSOLVERやXLA  
  - 機械学習・深層学習の文脈でよく使われる

__2. 注意点__

__2-1. 実装の違い（完全SVD vs 近似SVD）__
- **NumPy/SciPy**：完全なSVD（全特異値・特異ベクトル）  
- **Scikit-learnの`randomized_svd`**：指定ランク以下の近似SVD（高速だが厳密でない）

用途に応じて使い分けが必要です。

__2-2. メモリと速度のトレードオフ__
- **`gesdd`（分割征服）**：高速だがメモリ使用量が多い  
- **`gesvd`（標準）**：やや遅いがメモリ効率が良い

大規模行列では `gesdd` が推奨されることが多いです。

__2-3. 疎行列・大規模行列__
- 密行列用の `numpy.linalg.svd` は、大規模疎行列には不向き  
- 疎行列向けには **`scipy.sparse.linalg.svds`**（ARPACKベースの部分SVD）を使う

__2-4. 数値安定性__
- LAPACKベースの実装は数値的に安定  
- ただし、条件数が極端に大きい行列（特異値が非常に小さい）では、特異ベクトルが不安定になる可能性あり

__2-5. GPU・自動微分__
- PyTorch/JAXのSVDは自動微分可能だが、**特異値が重複する場合の勾配に注意**  
- GPUメモリ制約があるため、巨大行列ではバッチ処理や近似SVDを検討
...
1. **QR分解**で問題を正方行列に縮小
2. **二重対角化**で行列を疎な二重対角形に変換
3. **二重対角行列のSVD**を高速・安定に計算（分割征服法など）
4. **逆変換**で元の行列のSVDを復元

この「QR → 二重対角化 → SVD」の流れは、Golub–Kahan型アルゴリズムとして広く用いられており、**数値的安定性と計算効率の両立**を実現しています。
...
**共通点**
- どちらも「行列（線形写像）の構造分解」手法。
- 「うまい基底を選ぶと対角（あるいは対角に近い形）になる」という発想。
- ランク・核・像といった基本構造を明らかにする。

**違い**
- **ジョルダン標準形**：一般の基底で「できるだけ対角に近い形（ブロック対角）」に分解。理論解析（微分方程式、一般化固有空間など）に強い。
- **SVD**：正規直交基底で「直交変換＋対角伸縮」に分解。ノルム・内積・近似・逆操作など、数値的・統計的な応用に強い。
...
A = U \Sigma V^*
$$
- $ U $：$ m \times m $ のユニタリ（直交）行列（左特異ベクトル）
- $ V $：$ n \times n $ のユニタリ（直交）行列（右特異ベクトル）
- $ \Sigma $：$ m \times n $ の「対角」行列（特異値 $ \sigma_1 \ge \dots \ge \sigma_r > 0 $、残り 0）

**存在証明の要点**
- $ A^*A $ はエルミート半正定値 → 固有値 $ \lambda_i \ge 0 $、固有ベクトル $ v_i $ は正規直交。
- 特異値 $ \sigma_i = \sqrt{\lambda_i} $、右特異ベクトル $ v_i $。
- 左特異ベクトル $ u_i = \frac{1}{\sigma_i} A v_i $ と定義すると正規直交。
- 残りの基底を補完して $ U, V $ をユニタリに拡張し、$ A = U \Sigma V^* $ を得る。

### 3. 幾何的意味と線形写像の構造

**幾何的意味**
- SVDは「回転（直交変換）→ 軸方向の伸縮 → 回転」の3ステップ分解：
  $$
  y = A x = U \Sigma V^* x
  $$
  1. $ V^* x $：入力空間を右特異ベクトル座標に回転
  2. $ \Sigma $：各軸方向を特異値ぶん伸縮（特異値 0 の方向はつぶれる）
  3. $ U $：出力空間を左特異ベクトル座標に回転

**線形写像の構造**
- ランク $ r $ ＝ 非ゼロ特異値の個数
- 核 $ \text{Ker}(A) = \text{span}\{v_{r+1}, \dots, v_n\} $（特異値 0 の右特異ベクトル）
- 像 $ \text{Im}(A) = \text{span}\{u_1, \dots, u_r\} $（非ゼロ特異値の左特異ベクトル）

**直交分解としての解釈**
- 入力空間：$ \mathbb{C}^n = \text{Im}(A^*) \oplus \text{Ker}(A) $
- 出力空間：$ \mathbb{C}^m = \text{Im}(A) \oplus \text{Ker}(A^*) $
- すべての基底が正規直交なので、SVDは核・像の**直交分解**を明示的に与える。

### 4. 代表的な応用

**低ランク近似（Eckart–Youngの定理）**
- SVDの上位 $ k $ 個の特異値・特異ベクトルだけを使った
  $$
  A_k = \sum_{i=1}^k \sigma_i u_i v_i^*
  $$
  は、ランク $ k $ の行列の中で $ A $ に最も近い近似。
- 誤差：$ \|A - A_k\|_F = \sqrt{\sum_{i=k+1}^r \sigma_i^2} $
- 画像圧縮・データ圧縮・ノイズ除去などに応用。

**擬似逆行列（Moore–Penrose逆行列）**
- SVD $ A = U \Sigma V^* $ に対し、
  $$
  A^\dagger = V \Sigma^\dagger U^*
  $$
  （非ゼロ特異値の逆数をとり、残りは 0）
- 最小二乗解：$ x^* = A^\dagger b $ は $ \min_x \|Ax - b\|^2 $ の**ノルム最小解**。
- 過剰決定・不足決定・ランク落ちの線形方程式に共通の「逆操作」を定義。

**主成分分析（PCA）**
- 中心化データ行列 $ \tilde{X} $ のSVD：$ \tilde{X} = U \Sigma V^\top $
- 右特異ベクトル $ V $ が主成分方向、特異値の2乗が分散に対応。
- 低次元表現：$ Z = \tilde{X} V_k $（上位 $ k $ 主成分への射影）。

**行列ノルムと条件数**
- 2-ノルム：$ \|A\|_2 = \sigma_1 $（最大特異値）
- フロベニウスノルム：$ \|A\|_F = \sqrt{\sum \sigma_i^2} $
- 核ノルム：$ \|A\|_* = \sum \sigma_i $
- 2-ノルム条件数：$ \kappa_2(A) = \sigma_1 / \sigma_n $

### 5. 数値計算アルゴリズム（概要）

**標準的な流れ（Golub–Kahan型）**
1. **QR分解**：$ A = Q R $ で問題を正方行列に縮小。
2. **二重対角化**：Householder変換で $ R $（または $ A $）を二重対角行列 $ B $ に変換（非ゼロは対角とその隣のみ）。
3. **二重対角行列のSVD**：分割征服法などで高速・安定に計算。
4. **逆変換**：$ B $ のSVDから元の $ A $ のSVDを復元。

**特徴**
- 計算コスト：$ O(mn^2) $（$ m \ge n $）
- 数値的に安定（直交変換ベース）
- LAPACK（`dgesvd` / `dgesdd`）や NumPy/SciPy の内部実装がこの流れ

**Pythonでの主な実装**
- `numpy.linalg.svd` / `scipy.linalg.svd`：完全SVD（LAPACK利用）
- `scipy.linalg.svd(..., lapack_driver='gesdd')`：分割征服法（高速・メモリ多め）
- `sklearn.utils.extmath.randomized_svd`：大規模行列向けの近似SVD
- `scipy.sparse.linalg.svds`：疎行列向けの部分SVD
- `torch.linalg.svd` / `jax.numpy.linalg.svd`：GPU・自動微分対応
