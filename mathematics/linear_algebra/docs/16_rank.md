...
行列のランクを「小行列式（Minor）」を使って定義する方法は、行列の成分から直接的にランクの値を決定できる、非常に美しく強力な数学的アプローチです。

直感的には、 **「行列の中に隠れている、最も大きな『潰れていない（逆行列を持つ）正方形のブロック』のサイズ」** が、その行列のランクである、という考え方です。


### 1. 小行列式（Minor）とは

行列 $A$ から、いくつかの行と列を適当に選んで取り除き、残った要素で構成される正方形の行列式を「小行列式」と呼びます。

- 例えば $3 \times 4$ 行列があるとき、そこから $2 \times 2$ の正方形部分を取り出して計算した行列式も小行列式の一つです。

__例:__


行列 $A$ があるとき、その中のどこか一部分を切り出したものだと考えると分かりやすいです。具体例で見てみましょう。

__1. 例題：$3 \times 3$ 行列からの小行列__
...
この行列 $A$ から、例えば **「1行目」と「2列目」を削除** してみます。

* 削除する行：1行目 $(1, 2, 3)$
* 削除する列：2列目 $(2, 5, 8)^T$

すると、残った要素は以下のようになります。
...
これが「$A$ の一部である $2 \times 2$ の小行列」です。

__小行列式（Minor）を作る__

先ほどの小行列の**行列式**を計算すると、「小行列式（Minor）」になります。

$$\det \begin{pmatrix}
4 & 6 \\
7 & 9
\end{pmatrix} = (4 \times 9) - (6 \times 7) = 36 - 42 = -6$$

この $-6$ が、行列 $A$ の特定の要素に対する小行列式となります。
...
行列 $A$ のランクは、以下の条件を満たすような**最大の整数 $r$** です。

1. **ランク $r$ の存在:** $A$ の中に、$r \times r$ の小行列式で、値が **$0$ ではないものが少なくとも1つ存在する**。
2. **ランク $r$ の上限:** $A$ の中のすべての $(r+1) \times (r+1)$ 小行列式を計算すると、それらが**すべて $0$ になる**。

つまり、 **「サイズ $r$ なら『まだ潰れていない部分』が見つかるが、サイズ $r+1$ にすると『すべての部分が潰れてしまう』」** という境界線が、その行列のランクとなります。
...
* **サイズ 2 の小行列式をチェック:**
* 左上の $2 \times 2$ 部分：$\det \begin{pmatrix} 1 & 2 \\ 2 & 4 \end{pmatrix} = 4 - 4 = 0$
* 右上の $2 \times 2$ 部分：$\det \begin{pmatrix} 2 & 3 \\ 4 & 6 \end{pmatrix} = 12 - 12 = 0$
* すべての $2 \times 2$ 小行列式が $0$ です。


* **サイズ 1 の小行列式をチェック:**
* 成分の「1」など、$0$ でないものが存在します。



したがって、この行列のランクは **1** となります。


### 4. なぜこの定義が重要なのか

この「小行列式による定義」には、他の定義（列の独立性など）にはないメリットがあります。

* **計算の理論的基盤:** 掃き出し法（ガウス消去法）を使わなくても、行列式さえ計算できればランクが決定できます。
* **行列式の性質とのリンク:** 「行列式が $0$ である $\iff$ ランクがフルではない」という関係性が、この定義によって一般化されています。
* **座標変換との相性:** 幾何学的に言えば、小行列式は「行列が作る平行多面体の体積」を測る道具です。体積が $0$ でない場所を探すことで、行列の「実力（ランク）」を測定しているわけです。


### まとめ

* **小行列式によるランク** ＝ 「$r$ 次の正方行列として、体積が $0$ にならない（潰れていない）最大のブロックのサイズ」。
* **掃き出し法（ガウス消去法）との違い:**
* 掃き出し法はランクを求めるための「実際の手続き」ですが、小行列式はランクを定義するための「理論的な基準」です。


__定理__

$m \times n$行列 $A$ の $n$個の列ベクトルが１次独立であるための必要十分条件は、$A$の$n$次の小行列式の値が0でないものが存在することである。

---
この定理は、行列の「列の独立性（ベクトル空間的性質）」と「小行列式（代数的性質）」を結びつける非常に重要なものです。

証明の方針としては、 **「列ベクトルが1次独立である $\iff$ ランクが $n$ である」** という事実と、以前触れた **「小行列式によるランクの定義」** を橋渡しすることで導きます。

__証明__

__1. 必要性の証明 ($\implies$)__

**仮定：** $A$ の $n$ 個の列ベクトル $\mathbf{a}_1, \dots, \mathbf{a}_n$ が1次独立であるとする。

このとき、行列 $A$ のランクの定義より、$\text{rank} A = n$ である。
ランクの「小行列式による定義」によれば、ランクが $n$ であることは、 **「少なくとも1つの $n$ 次の小行列式が $0$ ではない」** ことと同値である。
（もし全ての $n$ 次小行列式が $0$ ならば、ランクは $n$ 未満になってしまい、仮定に矛盾する。）

したがって、$0$ でない $n$ 次の小行列式が存在する。

__2. 十分性の証明 ($\impliedby$)__

**仮定：** $A$ の $n$ 次の小行列式の値が $0$ でないものが少なくとも1つ存在するとする。

その $0$ でない $n$ 次小行列式に対応する $n \times n$ の小行列を $M$ とする。
$\det M \neq 0$ であることから、行列 $M$ の $n$ 個の列ベクトルは1次独立である。

$M$ の列ベクトルは、元の行列 $A$ の各列ベクトルから特定の行（成分）を抜き出したものである。
もし元の $A$ の列ベクトルが1次従属であれば、その一部を抜き出した $M$ の列ベクトルも必然的に1次従属になるはずである（線形結合の係数がそのまま使えるため）。

しかし、いま $M$ の列ベクトルは1次独立であるため、元の $A$ の $n$ 個の列ベクトルも1次独立でなければならない。
...
この証明が直感的に何を言っているかというと、 **「全体が独立しているなら、どこか一部に『潰れていない正方形』が隠れているはずだ」** ということです。

* **列ベクトルが1次独立:** $n$ 個のベクトルが $n$ 次元の広がりをしっかり持っている。
* **$n$ 次小行列式が非ゼロ:** $m$ 個ある成分のうち、適切な $n$ 個の「視点（行）」を選べば、その広がりが正方形の「体積」として観測できる。


__なぜこの定理が「AIや科学計算」で重要なのか？__

この定理は、データの **「冗長性」** を判定する基準になります。

* **AIの文脈:** 入力データの次元が $n$ あっても、全ての $n$ 次小行列式が $0$ ならば、そのデータには実質的に $n$ 未満の情報しかない（多重共線性がある）ことがわかります。
* **計算の安定性:** 小行列式の値が $0$ に極めて近い場合、数値計算において逆行列を求める際に大きな誤差（丸め誤差）が生じやすくなります。
