...
実計量ベクトル空間においては一般に**直交変換 (Orthogonal Transformation)**、複素計量ベクトル空間（エルミート内積空間）においては**ユニタリ変換 (Unitary Transformation)** と呼ばれます。
...
### 2. なぜ計量ベクトル空間で重要なのか？
計量ベクトル空間において「長さ」と「角度」は最も本質的な情報です。直交行列はこの情報を一切壊さないため、以下のような場面で重宝されます。

* **座標変換の安定性:**
    データの基底（軸）を直交行列で変換しても、データの相対的な位置関係や距離が保たれるため、数値計算において非常に安定しています。
* **行列の分解（QR分解や特異値分解）:**
    複雑な行列を「直交行列 × 扱いやすい行列（上三角行列や対角行列）」に分解することで、方程式の解法や次元削減（PCAなど）が効率化されます。
