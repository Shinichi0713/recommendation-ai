...
- **特異値分解との関係**  
  任意の行列 $A$ に対して $A^H A$ や $A A^H$ はエルミット行列であり、その固有値の平方根が特異値になります。
...
ユニタリ行列は「ベクトルの長さと内積を保つ」という性質から、**幾何学的な回転・鏡映・長さを変えない変換**を表すのに適しており、以下のような分野で広く利用されています。

__1. 量子力学__

- **時間発展演算子**  
  量子系の時間発展はユニタリ演算子（無限次元版のユニタリ行列）で記述されます：
  $$
  |\psi(t)\rangle = U(t) |\psi(0)\rangle,\quad U(t) = e^{-iHt/\hbar}
  $$
  ここで $H$ はエルミートなハミルトニアンです。  
  ユニタリであるため、確率の総和（ノルム）が保存されます。

- **量子ゲート**  
  量子コンピュータでは、量子ビットに対する操作（量子ゲート）はユニタリ行列で表されます。  
  例：パウリゲート、アダマールゲート、CNOTゲートなど。

- **基底変換**  
  量子状態を別の基底（位置表示・運動量表示など）に変換する操作もユニタリ変換です。

__2. 信号処理・フーリエ解析__

- **離散フーリエ変換（DFT）行列**  
  $N$ 点の離散フーリエ変換は、ユニタリ行列 $F$ を用いて
  $$
  \mathbf{X} = F \mathbf{x}
  $$
  と書けます（適切な正規化をすれば $F^H F = I$）。  
  これにより、時間領域の信号を周波数領域に変換できます。

- **ウェーブレット変換**  
  多くのウェーブレット変換もユニタリ（または直交）変換として定式化され、信号の多解像度解析に使われます。

__3. 線形代数・数値線形代数__

- **スペクトル分解・対角化**  
  正規行列（エルミート行列やユニタリ行列など）はユニタリ行列で対角化できます：
  $$
  A = U \Lambda U^H
  $$
  ここで $U$ はユニタリ行列、$\Lambda$ は固有値の対角行列です。

- **QR分解**  
  グラム・シュミットの直交化やハウスホルダー変換を通じて、任意の行列を直交（ユニタリ）行列 $Q$ と上三角行列 $R$ の積
  $$
  A = QR
  $$
  に分解します。$Q$ は実数なら直交行列、複素数ならユニタリ行列です。
...
- **特異値分解（SVD）**  
  ...
  A = U \Sigma V^H
  $$
  と分解できます。ここで $U, V$ はユニタリ行列、$\Sigma$ は特異値の対角行列です。

__4. 群論・幾何学__

- **ユニタリ群 $U(n)$**  
  $n$ 次ユニタリ行列全体は群をなし、コンパクト李群として研究されます。  
  これは複素ベクトル空間における「回転＋鏡映」の群に対応します。

- **特殊ユニタリ群 $SU(n)$**  
  行列式が 1 のユニタリ行列のなす群は、素粒子物理のゲージ理論などで重要な役割を果たします。

__5. 情報理論・符号理論__

- **ユニタリ誤差**  
  量子情報では、ノイズや誤差をユニタリ変換としてモデル化することがあります。

- **ユニタリ符号（unitary codes）**  
  量子誤り訂正符号の設計において、ユニタリ行列の性質を利用した符号構成が研究されています。

__6. 制御理論・ロボティクス__

- **姿勢表現**  
  3次元空間での回転は直交行列（実ユニタリ行列）で表されます。  
  ロボットの関節角度や姿勢の変換に直交行列が使われます。

- **安定性解析**  
  線形システムの状態遷移行列がユニタリ（または直交）であれば、系のエネルギーが保存されるような保守系として解析できます。
...
複素ベクトルに対してユニタリ行列を作用させた様子をPythonで可視化します。
複素2次元ベクトルを「実部・虚部を分けた4次元実ベクトル」とみなし、そのうちの2成分（第1成分の実部・虚部）を平面に描画します。
...
<img src="image/18_inner_product/1774744839087.png" width="500" style="display: block; margin: 0 auto;">

__コードのポイント__

- `U` は対角成分が $e^{i\theta}, e^{-i\theta}$ のユニタリ行列です：
  $$
  U = \begin{pmatrix}
  e^{i\theta} & 0 \\
  0 & e^{-i\theta}
  \end{pmatrix}
  $$
- `U.conj().T @ U` が単位行列に近いことを確認し、ユニタリ性（$U^H U = I$）を数値的に確かめています。
- 複素ベクトル `z` と `Uz` のノルムが同じであることも表示し、「長さを保つ変換」であることを確認しています。
- 可視化では、**第1成分の実部・虚部**を2次元平面にプロットしています。  
  この例では、`z[0] = 1+0j` が `(Uz)[0] = e^{i\theta}` に回転され、単位円上を動く様子が見えます。

__複素ベクトル空間のイメージ__

- 複素2次元ベクトル空間 $\mathbb{C}^2$ は、実4次元空間 $\mathbb{R}^4$ と同型です。
- ここではそのうち「第1成分の実部・虚部」だけを取り出して2次元平面に描画しています。
- ユニタリ行列は、この4次元実空間内で「回転＋鏡映」のような、長さを保つ変換を引き起こします。

このコードを実行すると、**複素ベクトルの成分が単位円上を回転する**様子が視覚的に確認できます。



## 本章まとめ

計量ベクトル空間と内積・外積・直交性・ユニタリ行列の要点を、コンパクトに整理します。


### 1. 計量ベクトル空間：長さと角度を測る舞台

- **計量（Metric）**：空間に「長さ」と「角度」を定義するルール。
- **内積**：計量の実体。  
  標準的には $\langle \mathbf{x}, \mathbf{y} \rangle = \mathbf{x}^T G \mathbf{y}$（$G$ は計量行列）。
- **ノルム（長さ）**：$\|\mathbf{x}\| = \sqrt{\langle \mathbf{x}, \mathbf{x} \rangle}$。
- **なす角**：$\cos\theta = \frac{\langle \mathbf{x}, \mathbf{y} \rangle}{\|\mathbf{x}\|\|\mathbf{y}\|}$。

> 計量が入ることで、ベクトル空間は「幾何学的な舞台」になる。

### 2. 内積と外積：重なりと回転の道具

- **内積（Inner / Dot Product）**  
  - スカラーを返す：$\mathbf{a}\cdot\mathbf{b} = |\mathbf{a}||\mathbf{b}|\cos\theta$。  
  - 成分表示：$\mathbf{a}\cdot\mathbf{b} = a_1b_1 + a_2b_2 + \dots$（正規直交基底なら）。  
  - 役割：射影（影の長さ）、直交判定、類似度（コサイン類似度）。

- **外積（Cross Product）**（主に3次元）  
  - ベクトルを返す：$\mathbf{a}\times\mathbf{b}$。  
  - 大きさ：平行四辺形の面積 $|\mathbf{a}\times\mathbf{b}| = |\mathbf{a}||\mathbf{b}|\sin\theta$。  
  - 方向：$\mathbf{a}$ と $\mathbf{b}$ の両方に垂直（右ねじの法則）。  
  - 役割：法線ベクトル、トルク、面積計算。

### 3. 直交性と正規直交基底

- **直交**：$\langle \mathbf{x}, \mathbf{y} \rangle = 0$。
- **正規直交基底**：  
  - 互いに直交し、長さが 1 の基底ベクトルの組。  
  - 内積計算が「成分の積の和」に簡略化される。
- **グラム・シュミットの正規直交化法**：  
  任意の独立なベクトル列から正規直交基底を作るアルゴリズム。
- **直交補空間 $W^\perp$**：  
  $W$ のすべてのベクトルと直交するベクトルの集合。  
  空間は $V = W \oplus W^\perp$ と直和分解できる。
...
- **直交行列（実数）**：  
  - $Q^T Q = I$（$Q^{-1} = Q^T$）。  
  - 列ベクトルが正規直交基底。  
  - 行列式 $\det(Q) = \pm 1$（回転 or 反転）。  
  - 応用：座標変換、QR分解、PCA、画像圧縮（DCT）など。
- **ユニタリ行列（複素数）**：  
  - $U^H U = I$（$U^{-1} = U^H$）。  
  - 固有値の絶対値は 1（単位円上）。  
  - 応用：量子力学（時間発展演算子）、フーリエ変換、SVDなど。

### 5. 計量同型：次元が同じなら「同じ空間」

- **計量同型**：  
  2つの計量ベクトル空間が、線形構造と計量構造の両方を完全に保存する写像で結ばれている状態。
- **定理**：  
  次元が等しい有限次元の計量ベクトル空間は、必ず計量同型。  
  → 次元が同じなら、中身が関数でも行列でも、$\mathbb{R}^n$（標準内積空間）として扱ってよい。

### 6. エルミット内積とエルミット行列（複素数版）

- **エルミット内積**：  
  $\langle \mathbf{x}, \mathbf{y} \rangle = \sum x_i \overline{y_i}$。  
  共役対称性 $\langle \mathbf{x}, \mathbf{y} \rangle = \overline{\langle \mathbf{y}, \mathbf{x} \rangle}$ により、ノルムが常に実数になる。
- **エルミット行列**：$A^H = A$。  
  - 固有値はすべて実数。  
  - 固有ベクトルは正規直交基底をなす。  
  - 量子力学の観測量、共分散行列、SVDの基盤。


### 7. 全体のストーリー

- **計量**を入れることで、ベクトル空間は「長さ・角度・距離」が測れる幾何空間になる。
- **内積**は「重なり具合」を測り、**外積**は「回転と面積」を扱う。
- **直交性**と**正規直交基底**により、計算が単純化され、データの独立成分が抽出できる。
- **直交行列・ユニタリ行列**は、計量を保つ「回転・反転」の変換であり、  
  数値安定性・次元圧縮・量子計算・信号処理の基盤となる。
- **計量同型**により、次元が同じ空間は本質的に同じ構造を持つことが保証される。
...
- 量子状態の時間発展  
- 行列の安定な分解（QR・SVD）  
など、数学・物理・工学・AIの幅広い問題を統一的に扱うことができます。
