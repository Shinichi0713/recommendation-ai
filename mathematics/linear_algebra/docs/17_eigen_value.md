...
本章では固有値（Eigenvalue）を扱います。
本章で扱う固有値・固有ベクトルは、行列（変換）の「本質的な動き」を抽出する道具です。
固有値を理解すると、
- 「この変換は、どの方向にどれだけ伸び縮みするのか？」
- 「このシステムは安定か？ 振動するか？」

これが「固有値」の基本的なイメージと、学ぶ意義です。

## 概念
固有値とは、一言で言えば **「行列という『変換』を施しても、方向が変わらず、長さだけが変化する特別なベクトルの『伸び率』」** のことです。
...
<img src="image/17_eigen_value/1773176361220.png" width="500" style="display: block; margin: 0 auto;">
...
数値計算ライブラリの **NumPy** を使えば、固有多項式の係数の算出から行列の代入まで実装できます。
...
```
固有多項式の係数: [ 1.00000000e+00 -1.50000000e+01 -1.80000000e+01 -2.34661991e-14]
...
次は固有値、固有ベクトルのメリットが分かるPython例題をやってみます。

人口移動の長期予測設定:
- 都市の人口 $u$ と、地方の人口 $v$ があります。
- 毎年、都市の人の 10% が地方へ移住し、地方の人の 20% が都市へ移住します。
- この移動ルールを行列 $A$（推移行列）で表すと、翌年の人口は $A \begin{pmatrix} u \\ v \end{pmatrix}$ となります。
 
__固有値・固有ベクトルの便利ポイント:__
...
```
100年後の人口（逐次計算）: 都市 666.7人, 地方 333.3人
最終的な人口（固有ベクトル）: 都市 666.7人, 地方 333.3人
```
...
100回、1000回と行列を掛け算しなくても、「固有値 1」に対応する固有ベクトルを求めるだけで、システムが最終的にどこに落ち着くか（定常状態）がわかります。

__2. システムの性質の理解:__

固有値を見れば、そのシステムが「拡大していくのか（固有値 > 1）」「消滅するのか（固有値 < 1）」「安定するのか（固有値 = 1）」が計算前にわかります。

__3. GoogleのPageRankの原理:__

実は、Googleが初期に採用していた検索アルゴリズム「PageRank」もこれと同じ原理です。ウェブサイトを移動し続ける「ネットサーファー」が最終的にどのページに多く滞留するかを、巨大な行列の固有ベクトル計算（べき乗法）で求めています。

実装上のアドバイス
- np.linalg.eig で得られる固有ベクトルは、通常長さが 1 に正規化されています。

- 今回のように「合計人数」が重要な場合は、ベクトルの成分の和で割ってから、総人口を掛けることで具体的な数値に戻せます。

## 総括

固有値・固有ベクトルとその周辺概念（対角化・直交行列・三角化・行列多項式）を、行列の「本質的な動き」を抽出する道具としてまとめると次のようになります。

### 1. 固有値・固有ベクトル：行列の「不動の軸」と「伸び率」
...
- **定義**  
  ...
  $$
  を満たすスカラー $\lambda$ を**固有値**、ベクトル $\mathbf{v}$ を**固有ベクトル**と呼ぶ。

- **幾何的意味**  
  - 固有ベクトル：行列を掛けても向きが変わらない「不動の軸」  
  - 固有値：その軸方向の「伸び縮みの倍率」

- **なぜ重要か**  
  - 行列の累乗 $A^n$ が「固有値の $n$ 乗」で簡単に計算できる（対角化）  
  - システムの安定性（収束・発散）や共振（固有振動数）を判定できる  
  - データの主成分（情報の多い方向）を抽出できる（PCA）


### 2. 対角化：行列を「固有値のリスト」に分解する

- **対角化の式**  
  固有ベクトルを列に並べた正則行列 $P$ と、固有値を対角成分に並べた対角行列 $D$ を用いて  
  $$
  P^{-1}AP = D
  $$
  と書けるとき、$A$ は**対角化可能**。
...
- **対角化の条件**  
  - $n$ 次正方行列が $n$ 本の線形独立な固有ベクトルを持つこと  
  - 各固有値で「代数的重複度＝幾何学的重複度」が成り立つこと

- **メリット**  
  - $A^n = P D^n P^{-1}$ で、累乗が「固有値の $n$ 乗」だけで計算できる  
  - 複雑なシステムを「独立なモード（固有ベクトル方向）」の足し算として扱える


### 3. 直交行列：長さと角度を変えない「回転・反転」の行列
...
- **定義**  
  ...
  $$
- **性質**  
  - ベクトルの長さ（ノルム）と内積（角度）を保つ  
  - 行列式は $\det(Q) = \pm 1$（回転 or 反転）  
  - 列ベクトルが正規直交基底をなす

- **役割**  
  - PCAでデータを「情報の多い方向」に回転させる  
  - QR分解などで行列を「直交成分」と「三角成分」に分解する

### 4. 三角化：対角化できない行列も「上三角」に整理する
...
- **三角化の式**  
  ...
  P^{-1}AP = T
  $$
  とし、$T$ を**上三角行列**（対角成分より下がすべて 0）にする。

- **シューアの定理**  
  複素数の範囲では、**任意の正方行列は三角化可能**。  
  対角成分には固有値が並ぶ。

- **メリット**  
  - 対角化できない行列でも、固有値が対角成分として見える  
  - 行列式が「対角成分の積」で簡単に計算できる

### 5. 行列の多項式とケーリー・ハミルトンの定理

- **行列多項式**  
  多項式 $f(x)$ の $x$ に行列 $A$ を代入したもの：  
  $$
  f(A) = a_n A^n + \dots + a_1 A + a_0 I
  $$
- **固有値との関係**  
  $A\mathbf{v} = \lambda \mathbf{v}$ なら $f(A)\mathbf{v} = f(\lambda)\mathbf{v}$  
  → $f(A)$ の固有値は $f(\lambda)$、固有ベクトルは共通

- **ケーリー・ハミルトンの定理**  
  行列 $A$ の固有多項式を $\Phi_A(\lambda)$ とすると、  
  $$
  \Phi_A(A) = O \quad (\text{零行列})
  $$
  → 行列は「自分自身の方程式」を満たす。
...
- **応用**  
  - 高次のべき乗 $A^n$ を低次式に「次数下げ」して計算  
  - 逆行列を $A$ の多項式として表現できる

### 6. 固有値の重複度と対角化の関係

- **代数的重複度**：固有多項式の解としての重なり数（予約枠）  
- **幾何学的重複度**：固有空間の次元（実際の固有ベクトルの本数）

- **関係**  
  $ 1 \leq \text{幾何学的重複度} \leq \text{代数的重複度} $
- **対角化可能**  
  ⇔ すべての固有値で「代数的重複度＝幾何学的重複度」  
  ⇔ $n$ 本の線形独立な固有ベクトルが存在

### 7. 全体のまとめ

- **固有値・固有ベクトル**は、行列（線形変換）の「本質的な動き」を  
  「どの方向にどれだけ伸び縮みするか」という形で抽出する道具。
- **対角化**は、その本質を「対角行列」という最も単純な形にまとめる操作。
- **直交行列**は、ベクトルの長さと角度を保ちながら座標系を回転させる「綺麗な視点」。
- **三角化**は、対角化できない行列でも「上三角」に整理して固有値を可視化する方法。
- **行列多項式とケーリー・ハミルトン**は、行列のべき乗や逆行列を  
  固有値と多項式の言葉でスマートに扱うための理論的土台。

これらを組み合わせることで、  
- 連立方程式の長期挙動（人口移動・マルコフ連鎖）  
- システムの安定性・共振  
- データの主成分分析・次元圧縮  
など、数学・物理・工学・データ科学の幅広い問題を統一的に扱うことができます。
