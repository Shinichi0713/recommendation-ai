...
**CRD (Collaborative Representation-based Detector)** は、ハイパースペクトル画像（HSI）において**「周囲の画素（背景）で自分を説明できるか？」**を問うことで異常を検知する、非常にスマートなアルゴリズムです。

従来の RX Detector（統計モデル）とは異なり、事前の統計分布（正規分布など）を仮定しないため、背景が複雑な場所でも高い精度を発揮します。

---

### CRDの核心：「背景の仲間外れ」を探す理論

CRDの基本的な考え方は、**「背景画素は、その近所の背景画素たちの足し合わせ（線形結合）で表現できるはずだ」**という仮定に基づいています。

1. **注目画素（テスト画素）**とその**周辺（二重窓の内部）**に注目します。
2. 周辺にある「背景候補」の画素たちを組み合わせて、注目画素を再現（近似）しようと試みます。
3. **もし再現できたら**：その画素は周辺と同じ性質を持つ「背景」とみなします。
4. **もし再現できなかったら**：その画素は周辺とは異なる「異常（アノマリー）」とみなします。
...
* **統計分布に依存しない**:
RX検知器は背景が「ガウス分布（正規分布）」に従うと仮定しますが、実際の地表は草地、アスファルト、土などが混ざっており、分布が複雑です。CRDはデータそのものから再現性を計算するため、こうした複雑な背景に強いです。
* **協調的（Collaborative）な表現**:
「辞書（周辺画素）」全体を使ってターゲットを表現しようとします。これにより、特定の1つの画素に似ているかどうかではなく、背景全体のパターンに馴染むかどうかを判定できます。
* **$\ell_2$ ノルムによる高速化**:
数学的に最小二乗法に近い計算（$\ell_2$ 正則化）を用いるため、同様の考え方を持つスパース表現（SRD）などに比べて計算が非常に高速で、実用性が高いです。
...
| 特徴 | RX Detector | CRD |
| --- | --- | --- |
| **判定基準** | 平均からどれだけ離れているか（マハラノビス距離） | 周辺画素でどれだけ「再現」できるか（残差） |
| **背景モデル** | 多次元正規分布を仮定 | 特定の分布を仮定しない（ノンパラメトリック） |
| **得意な場面** | 背景が均一で単純な場合 | 背景に複数の物質が混ざっている複雑な場合 |

---

### なぜ前処理（フィルタリング）が重要か

CRDは周辺画素との関係性を見るため、画像に**空間的なノイズ**があると、再現計算が不安定になります。

* **空間フィルタの役割**:
先ほど実装した **ガウシアンローパス** や **ガイデッドフィルタ** をかけることで、背景画素同士の「仲間意識（相関）」が強まります。
* **効果**:
背景がより「背景らしく」滑らかになるため、異常な物体だけが「再現できない異物」としてより鮮明に浮き上がるようになります。

---

### 次のステップへの提案

CRDの実装は少し数学的（行列演算）になりますが、Python（NumPy）で記述可能です。

「フィルタリング済みのHSIデータ」を使って、**CRDによる異常検知を実行し、RXの結果と比較するコード**を作成しましょうか？これによって、手法による検知精度の違いを直接体験できます。
...
CRD（協調表現）は、LRXなどの統計的手法に比べて背景の複雑さに強い一方で、 **「背景でターゲットを再現しようとする」** というその仕組みゆえに、特定の条件下で検出力が極端に低下することがあります。

CRDで検出できない場合に考えられる主な原因を4つに整理しました。

### 1. 背景辞書による「ターゲットの完全再現」

CRDのスコアは「ターゲットと再構成（合成）結果の差」です。もし背景辞書（周囲の画素）を使って、ターゲットの色を完璧に作り出せてしまったら、スコアはゼロになります。

* **原因**: 異常の色が、周囲の背景色の「組み合わせ（線形結合）」で表現できてしまっている。
* **例**: 草原の中に「黄緑色」の物体がある場合。背景に「鮮やかな緑」と「枯れた黄色」があれば、その2つを混ぜることでターゲットの色を再現できてしまいます。
* **解決策**: 窓サイズを小さくして、余計な「素材」が辞書に入らないように制限するか、正則化パラメータ（）を大きくして、複雑な組み合わせを禁止します。

### 2. 背景辞書への「異常成分」の混入

LRXと同様に、CRDも背景窓の中に異常（ターゲットと同じような性質のもの）が含まれると弱くなります。

* **原因**: ターゲットと同じ（または似た）画素が背景辞書の中に入っている。
* **メカニズム**: CRDは「自分に似た素材」を辞書から見つけると、それを優先的に使って自分を再現します。辞書内に異常があれば、再構成誤差はほぼゼロになり、異常として浮き上がらなくなります。
* **解決策**: 内窓（ガード窓）を広げて、異常が背景窓に漏れ出さないようにします。

### 3. 正則化パラメータ（）の不適合

CRDは、係数（重み）の二乗和を抑える **Ridge回帰（正則化）** の形をとっています。

* ** が小さすぎる場合**: 辞書内の画素をどんな複雑な比率で組み合わせても良いことになり、ターゲットを「無理やり再現」してしまいます（過学習）。
* ** が大きすぎる場合**: どの画素も使えなくなり、背景そのものも再現できなくなります。結果として、画像全体がノイズだらけになり、異常が埋もれます。
* **解決策**:  を  から  程度の範囲で段階的に調整し、コントラストが最大になる点を探します。

### 4. 線形結合の限界（非線形な差異）

CRDは「素材の足し算」を前提としています。

* **原因**: 背景と異常の差が、単なる色の違いではなく、光の反射特性やテクスチャといった「非線形」な要素にある場合。
* **メカニズム**: ターゲットが背景の線形結合では説明しにくい複雑な特性を持っていても、背景が多次元（多バンド）であれば、数学的な「遊び」で近似できてしまうことがあります。
* **解決策**: カーネルCRD（KCRD）を検討するか、前述のセグメンテーション・ベースの手法と組み合わせて、比較対象をより純粋なものに絞り込みます。

### 原因を切り分けるためのチェック法

今の結果がどうなっているかによって、対策が変わります。

1. **画像全体が真っ暗（スコアが低い）**:
   → 背景でターゲットを再現しすぎています。窓サイズを小さくするか、$\lambda$ を大きくしてください。
2. **ノイズだらけで異常がどこか分からない**:
   → 正則化が効きすぎています。** を小さく**してください。
3. **異常の「縁（ふち）」だけが光っている**:
   → 内窓が小さすぎて、背景辞書に異常が混入しています。**内窓を大きく**してください。

オリジナル画像

![1772572247652](image/README/1772572247652.png)

ガイテッドフィルタ + CRD

![1772572216078](image/README/1772572216078.png)


## 方法の確認法


異常検知（特にCRDのような変化点検出）の結果を評価する方法について、一般的な評価指標と、MATLABからPythonへ移植した際の「正誤判定」の観点に分けて整理します。

---

## 1. 異常検知の一般的な評価指標

まず、異常検知タスクでは、通常「ラベル付きデータ（いつ異常が起きたか）」が少ないことが多いですが、もしラベルがあるなら、以下の指標が使えます。

### 1-1. 二値分類としての評価（ラベルあり）

- **混同行列（Confusion Matrix）**
  - TP（True Positive）：異常と予測し、実際に異常だった
  - FP（False Positive）：異常と予測したが、実際は正常
  - FN（False Negative）：正常と予測したが、実際は異常
  - TN（True Negative）：正常と予測し、実際も正常

- **Precision（適合率）**
  - 「異常と予測したもののうち、本当に異常だった割合」
  - Precision = TP / (TP + FP)
...
- **ROC-AUC**
  - しきい値を変えたときの「真陽性率（TPR = Recall）」と「偽陽性率（FPR = FP / (FP + TN)）」の関係をプロットした曲線の下の面積
  - モデル全体の「異常検出能力」を評価するのに有効です。

### 1-2. 変化点検出（CRD）特有の評価の考え方

CRDのような変化点検出では、「**何サンプル以内の誤差なら許容するか**」を決めるのが一般的です。

- **許容誤差ウィンドウ（tolerance window）**
  - 例：真の変化点 ± 5サンプル以内で検出されていれば「正解」とみなす
  - これにより、FP/FNをより現実的にカウントできます。

- **平均検出遅延（Average Detection Delay）**
  - 実際の変化点から、検出点までの時間（サンプル数）の平均
  - 小さければ小さいほど「素早く異常を検出できている」と言えます。

- **偽アラーム率（False Alarm Rate）**
  - 単位時間あたりの誤検出数
  - 実運用では「誤検出が多すぎると現場が疲弊する」ため、この指標も重要です。

---

## 2. MATLAB → Python移植時の「正誤判定」の方法

「移植が正しくできたか」を確認するには、**同じ入力に対してMATLABとPythonの出力が一致するか**を確認するのが最も確実です。

### 2-1. テストデータの準備

1. **単純な人工データ**
   - ステップ変化、ランプ変化、周期的な変化＋外れ値など、CRDが得意とするパターンを人工的に生成
   - 例：
     - 0〜100サンプル：平均0
     - 101〜200サンプル：平均+10（ステップ変化）
     - 201〜300サンプル：再び平均0
   - これで「変化点がどこにあるか」が明確に分かるので、評価がしやすいです。

2. **既知の変化点を持つ実データ**
   - もし利用可能なら、過去のデータで「いつ異常が起きたか」が分かっているものを使うと、より現実的な評価ができます。

### 2-2. MATLABとPythonの出力比較

1. **同じパラメータ設定で実行**
   - ウィンドウ幅、しきい値、スコア計算方法など、すべて同じにします。
...
2. **出力項目の比較**
   - CRDの典型的な出力は：
     ...
     - 異常フラグ（0/1）の時系列
     - 変化点のインデックスリスト
   - これらをMATLABとPythonで比較します。

3. **比較の方法**
   - **数値的一致チェック**
     - 例：`np.allclose(matlab_scores, python_scores, rtol=1e-5, atol=1e-8)` など
   - **変化点位置の一致チェック**
     - 許容誤差ウィンドウ（例：±1サンプル）を設けて比較
   - **可視化**
     - 元データ＋MATLABのスコア＋Pythonのスコアを同じ図にプロットし、目視で確認

### 2-3. ランダム性・初期化の扱い

- CRDにランダム性（例：ブートストラップ、モンテカルロ）が含まれる場合：
  - 乱数シードをMATLAB・Pythonで同じ値に固定して比較
  - NumPy / MATLABの乱数生成アルゴリズムが異なる場合は、移植時に注意が必要です。

---

## 3. 実運用を想定した評価（ラベルが少ない場合）

もしラベルがほとんどない、あるいは全くない場合は、以下のような間接的な評価も有用です。

- **既知の異常イベントでの検出確認**
  - 過去に「明らかに異常だった」と分かっているイベントで、CRDが反応しているか確認

- **専門家レビュー**
  - 検出された異常候補をドメイン専門家に見せ、「本当に異常か」「見逃している異常はないか」を評価してもらう

- **偽陽性の影響評価**
  - 誤検出がどれだけ業務に影響するか（アラート疲れ、無駄な点検コストなど）を定性的に評価

---

## 4. まとめ

- **評価指標**としては、ラベルがある場合はPrecision/Recall/F1/ROC-AUC、変化点検出なら許容誤差ウィンドウや検出遅延も併用するのが良いです。
- **MATLAB → Python移植の正誤判定**としては、
  - 同じ人工データ・実データで、
  - 同じパラメータ・乱数シードで、
  - MATLABとPythonの出力（スコア・変化点位置）を数値的・可視的に比較する
  のが最も確実です。

もし、具体的なCRDアルゴリズム名や、出力形式（スコアの形、しきい値の決め方など）が分かれば、それに合わせたより具体的な評価方法もご提案できます。
