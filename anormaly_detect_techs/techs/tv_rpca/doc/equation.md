...
「RPCAの最適化関数にTV正則項がついたもの」という表現だけでは問題が一意に定まらないので、**典型的な RPCA + TV 正則化問題**を例に、ADMM の更新式を導出します。

---

## 1. 問題設定（RPCA + TV 正則化）

行列分解型の RPCA（Robust PCA）に TV 正則化を加えた問題を考えます：

\[
\min_{L, S} \; \|L\|_* + \lambda \|S\|_1 + \mu \,\mathrm{TV}(L) \quad \text{s.t.} \quad M = L + S
\]

ここで
- \(M \in \mathbb{R}^{m \times n}\)：観測行列（画像列など）
- \(L\)：低ランク成分
- \(S\)：スパース成分（外れ値・ノイズ）
- \(\|L\|_*\)：核ノルム（トレースノルム）
- \(\|S\|_1\)：要素ごとの L1 ノルム
- \(\mathrm{TV}(L)\)：行列 \(L\) を画像列と見なしたときの TV 正則化（各画像ごとに TV）
- \(\lambda, \mu > 0\)：正則化パラメータ

TV 項は、各列（各画像）に対して独立に TV をとる形とします（2次元画像の場合）：

\[
\mathrm{TV}(L) = \sum_{j=1}^n \mathrm{TV}(L_{\cdot,j}) = \sum_{j=1}^n \sum_{i} \sqrt{ (D_x L)_{i,j}^2 + (D_y L)_{i,j}^2 }
\]

ここで \(D_x, D_y\) は画像ごとの差分演算子です。

---

## 2. 補助変数の導入と拡張ラグランジュ関数

RPCA の制約 \(M = L + S\) に加え、TV 項を扱いやすくするために補助変数 \(P\) を導入します：

\[
\begin{aligned}
\min_{L, S, P} &\quad \|L\|_* + \lambda \|S\|_1 + \mu \|P\|_{2,1} \\
\text{s.t.} &\quad M = L + S,\quad P = D L
\end{aligned}
\]

ここで
- \(P = (P_x, P_y)\)：TV 用の補助変数（各画像の勾配場）
- \(\|P\|_{2,1} = \sum_{i,j} \sqrt{P_{x,i,j}^2 + P_{y,i,j}^2}\)（グループ L1 ノルム）
- \(D\)：勾配演算子（画像ごとに作用）
...
\[
\begin{aligned}
L_\rho(L, S, P, \Lambda, \Gamma)
&= \|L\|_* + \lambda \|S\|_1 + \mu \|P\|_{2,1} \\
&\quad + \langle \Lambda, M - L - S \rangle + \frac{\rho_1}{2} \|M - L - S\|_F^2 \\
&\quad + \langle \Gamma, P - D L \rangle + \frac{\rho_2}{2} \|P - D L\|_F^2
\end{aligned}
\]

です。ここで
- \(\Lambda\)：\(M = L + S\) に対する双対変数
- \(\Gamma\)：\(P = D L\) に対する双対変数
- \(\rho_1, \rho_2 > 0\)：ペナルティパラメータ

ADMM は、この \(L_\rho\) に対して以下の**交互最小化**を行います：

1. \(L\)-更新：\(S, P, \Lambda, \Gamma\) を固定して \(L_\rho\) を \(L\) について最小化
2. \(S\)-更新：\(L, P, \Lambda, \Gamma\) を固定して \(L_\rho\) を \(S\) について最小化
3. \(P\)-更新：\(L, S, \Lambda, \Gamma\) を固定して \(L_\rho\) を \(P\) について最小化
4. 双対変数の更新：\(\Lambda, \Gamma\) を更新

---

## 3. \(L\)-更新式の導出

\(S = S^k\), \(P = P^k\), \(\Lambda = \Lambda^k\), \(\Gamma = \Gamma^k\) を固定し、\(L\) について最小化します：

\[
\begin{aligned}
L^{k+1} &= \arg\min_L \; \|L\|_* \\
&\quad + \langle \Lambda^k, M - L - S^k \rangle + \frac{\rho_1}{2} \|M - L - S^k\|_F^2 \\
&\quad + \langle \Gamma^k, P^k - D L \rangle + \frac{\rho_2}{2} \|P^k - D L\|_F^2
\end{aligned}
\]

\(\Lambda^k, \Gamma^k\) に関する項をまとめると、

\[
\begin{aligned}
L^{k+1} &= \arg\min_L \; \|L\|_* \\
&\quad + \frac{\rho_1}{2} \|L - (M - S^k + \frac{\Lambda^k}{\rho_1})\|_F^2 \\
&\quad + \frac{\rho_2}{2} \|D L - (P^k + \frac{\Gamma^k}{\rho_2})\|_F^2
\end{aligned}
\]

これは**核ノルム正則化付き二次関数**の最小化であり、**特異値閾値作用素（singular value thresholding）**で解けます。

具体的には、まず二次項をまとめて

\[
L^{k+1} = \arg\min_L \; \|L\|_* + \frac{1}{2} \|A L - B\|_F^2
\]

の形にします（\(A, B\) は適当な線形演算子）。  
この問題の解は、行列 \(B\) の特異値分解 \(B = U \Sigma V^\top\) に対して

\[
L^{k+1} = U \operatorname{soft}_1(\Sigma) V^\top
\]

で与えられます（\(\operatorname{soft}_1\) はソフト閾値作用素）。

より具体的には、\(M - S^k + \frac{\Lambda^k}{\rho_1}\) と \(P^k + \frac{\Gamma^k}{\rho_2}\) を組み合わせた線形最小二乗問題を解き、その解に対して特異値閾値を適用する形になりますが、実装上は**反復的に解く**ことが多いです（例えば共役勾配法などで内側の二次最小化を行い、その結果に特異値閾値を適用）。

---

## 4. \(S\)-更新式の導出

\(L = L^{k+1}\), \(P = P^k\), \(\Lambda = \Lambda^k\), \(\Gamma = \Gamma^k\) を固定し、\(S\) について最小化します：

\[
S^{k+1} = \arg\min_S \; \lambda \|S\|_1 + \langle \Lambda^k, M - L^{k+1} - S \rangle + \frac{\rho_1}{2} \|M - L^{k+1} - S\|_F^2
\]

\(\Lambda^k\) に関する項をまとめると、

\[
S^{k+1} = \arg\min_S \; \lambda \|S\|_1 + \frac{\rho_1}{2} \|S - (M - L^{k+1} + \frac{\Lambda^k}{\rho_1})\|_F^2
\]

ここで \(W_S = M - L^{k+1} + \frac{\Lambda^k}{\rho_1}\) とおくと、

\[
S^{k+1} = \arg\min_S \; \lambda \|S\|_1 + \frac{\rho_1}{2} \|S - W_S\|_F^2
\]

これは**L1 ノルム付き二次関数**の最小化であり、**要素ごとのソフト閾値作用素**で解けます：

\[
S^{k+1} = \operatorname{soft}_{\lambda/\rho_1}(W_S)
\]

成分ごとに

\[
S_{ij}^{k+1} = \operatorname{sign}(W_{S,ij}) \max\bigl(|W_{S,ij}| - \frac{\lambda}{\rho_1},\; 0\bigr)
\]
...
\(L = L^{k+1}\), \(S = S^{k+1}\), \(\Lambda = \Lambda^k\), \(\Gamma = \Gamma^k\) を固定し、\(P\) について最小化します：

\[
P^{k+1} = \arg\min_P \; \mu \|P\|_{2,1} + \langle \Gamma^k, P - D L^{k+1} \rangle + \frac{\rho_2}{2} \|P - D L^{k+1}\|_F^2
\]

\(\Gamma^k\) に関する項をまとめると、

\[
P^{k+1} = \arg\min_P \; \mu \|P\|_{2,1} + \frac{\rho_2}{2} \|P - (D L^{k+1} - \frac{\Gamma^k}{\rho_2})\|_F^2
\]

ここで \(W_P = D L^{k+1} - \frac{\Gamma^k}{\rho_2}\) とおくと、

\[
P^{k+1} = \arg\min_P \; \mu \|P\|_{2,1} + \frac{\rho_2}{2} \|P - W_P\|_F^2
\]

これは**グループ L1 ノルム付き二次関数**の最小化であり、**ベクトルソフト閾値作用素**で解けます：

各画素位置 \((i,j)\) ごとに

\[
P_{i,j}^{k+1} = \operatorname{vecsoft}_{\mu/\rho_2}(W_{P,i,j})
\]
...
P_{i,j}^{k+1} = 
\begin{cases}
\left(1 - \frac{\mu/\rho_2}{\|W_{P,i,j}\|_2}\right) W_{P,i,j} & \|W_{P,i,j}\|_2 > \frac{\mu}{\rho_2} \\
0 & \|W_{P,i,j}\|_2 \le \frac{\mu}{\rho_2}
\end{cases}
\]
...
\[
P_{i,j}^{k+1} = \frac{W_{P,i,j}}{\|W_{P,i,j}\|_2} \max\bigl(\|W_{P,i,j}\|_2 - \frac{\mu}{\rho_2},\; 0\bigr)
\]
...
双対変数は、それぞれの制約の違反量に比例して更新されます：

\[
\begin{aligned}
\Lambda^{k+1} &= \Lambda^k + \rho_1 (M - L^{k+1} - S^{k+1}) \\
\Gamma^{k+1} &= \Gamma^k + \rho_2 (P^{k+1} - D L^{k+1})
\end{aligned}
\]

---

## 7. まとめ：RPCA + TV 正則化に対する ADMM 更新式

以上をまとめると、RPCA + TV 正則化問題

\[
\min_{L, S} \; \|L\|_* + \lambda \|S\|_1 + \mu \,\mathrm{TV}(L) \quad \text{s.t.} \quad M = L + S
\]

に対する ADMM の更新式は、補助変数 \(P = D L\) と双対変数 \(\Lambda, \Gamma\) を導入して

\[
\begin{aligned}
L^{k+1} &= \arg\min_L \; \|L\|_* + \frac{\rho_1}{2} \|L - W_L\|_F^2 + \frac{\rho_2}{2} \|D L - W_{DL}\|_F^2 \\
S^{k+1} &= \operatorname{soft}_{\lambda/\rho_1}\bigl( M - L^{k+1} + \frac{\Lambda^k}{\rho_1} \bigr) \\
P^{k+1} &= \operatorname{vecsoft}_{\mu/\rho_2}\bigl( D L^{k+1} - \frac{\Gamma^k}{\rho_2} \bigr) \\
\Lambda^{k+1} &= \Lambda^k + \rho_1 (M - L^{k+1} - S^{k+1}) \\
\Gamma^{k+1} &= \Gamma^k + \rho_2 (P^{k+1} - D L^{k+1})
\end{aligned}
\]
...
となります。ここで
- \(W_L = M - S^k + \frac{\Lambda^k}{\rho_1}\)
- \(W_{DL} = P^k + \frac{\Gamma^k}{\rho_2}\)
- \(\operatorname{soft}_{\tau}\)：要素ごとのソフト閾値作用素
- \(\operatorname{vecsoft}_{\tau}\)：ベクトルソフト閾値作用素
...
- \(L\)-更新は一般に閉形式では解けず、内側で反復解法（共役勾配法など）を用いて二次最小化を行い、その結果に対して特異値閾値を適用する、という**二段階の手順**になることが多いです。
- TV 正則化を加えることで、低ランク成分 \(L\) が「滑らかな構造」を持つように誘導されます。
- もし「RPCAの最適化関数にTV正則項がついたもの」が別の形（例えば TV が \(S\) にかかるなど）を指している場合は、目的関数の形に応じて補助変数の導入方法と更新式が変わりますが、**ADMM の基本方針（変数分離 → 交互最小化）**は同じです。
