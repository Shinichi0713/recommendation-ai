...
空間方向でRPCAを適用する場合、もとの3次元データ（画像キューブ）は一度**2次元行列に組み替え（フラット化）**られてから処理されます。

RPCAの核心は、全画素に共通する「背景の統計的構造」を見つけ出すことにあります。そのためには、波長（バンド）間の相関を利用できるようデータを配置する必要があります。

---

### 1. データの再構成（マトリクス化）

もとの画像キューブを $\mathcal{D} \in \mathbb{R}^{H \times W \times B}$ （$H$:高さ, $W$:幅, $B$:バンド数）とすると、これを2次元行列 $D \in \mathbb{R}^{B \times P}$ （$P = H \times W$:全画素数）に変換します。

* **行**: 各バンドのスペクトル成分
* **列**: 各画素のベクトル

このとき、行列 $D$ は次のように分解されます。


$$D = L + S$$

* **$L$（低ランク成分）**: 背景。画素間で共通するスペクトルパターンが支配的であるため、ランク（独立な列数）が低くなります。
* **$S$（スパース成分）**: 異常。特定の画素の特定のバンドで突発的に発生する値であり、非ゼロ成分が少ない（スパース）状態です。

---

### 2. 数式による最適化プロセス

RPCAは、以下の**主成分追跡（PCP: Principal Component Pursuit）**という最適化問題を解くことで $L$ と $S$ を分離します。

$$\min_{L, S} \|L\|_* + \lambda \|S\|_1 \quad \text{subject to} \quad L + S = D$$

各項の意味は以下の通りです。

* **$\|L\|_*$（核ノルム）**: 行列 $L$ の特異値の和。これを最小化することで、行列のランクを下げ、背景の「共通性」を抽出します。
* **$\|S\|_1$（$L_1$ノルム）**: 行列 $S$ の各要素の絶対値の和。これを最小化することで、ほとんどの要素を $0$ にし、稀に発生する「異常」だけを残します。
* **$\lambda$（ウェイト）**: 通常 $\lambda = 1/\sqrt{\max(B, P)}$ と定義され、低ランク性とスパース性のバランスを制御します。

---

### 3. アルゴリズム（ADMM）の更新式

先ほどの実装コードで用いたADMM（交互方向乗数法）では、以下のステップを繰り返します。

1. **低ランク成分の更新（特異値しきい値演算）**:

$$L_{k+1} = \text{SVD\_thresh}(D - S_k + \mu^{-1}Y_k, \mu^{-1})$$



これは特異値分解を行い、小さな特異値を $0$ に丸める操作です。
2. **スパース成分の更新（ソフトしきい値演算）**:

$$S_{k+1} = \text{Soft\_thresh}(D - L_{k+1} + \mu^{-1}Y_k, \lambda \mu^{-1})$$



これは小さな値の要素を $0$ に押し込み、大きな値（異常）だけを残す操作です。

---

### 4. 処理後の画像キューブへの復元

計算が終わると、行列 $L$ と $S$ はそれぞれ $(B, P)$ の形をしています。これを再びもとの $(H, W, B)$ にリシェイプ（形を戻す）することで、**「背景だけの画像キューブ」**と**「異常成分だけの画像キューブ」**が完成します。

* **異常検知のスコア**:
各画素 $p$ におけるスパース成分のエネルギーとして定義されます。

$$\text{Score}(p) = \sqrt{\sum_{b=1}^{B} S_{b, p}^2}$$
...
空間方向にRPCAをかけるということは、**「全画素のスペクトルを横並びにして、背景に馴染まないスペクトル変化を持つ画素をスパース成分として弾き出す」**という処理を、行列演算のレベルで行っていることになります。

この手法は、窓サイズを指定するRXやCRDと違い、**「画像全体の統計から浮いているもの」**を自動で見つけるため、空間的な解像度を維持したまま精度の高い検知が可能です。

次は、このRPCAの出力である **$S$ 成分（スパース成分）のスペクトル**を確認して、異常箇所がどのような「色」として分離されたかを分析してみますか？


$\lambda = 1e-3$

![1772745145572](image/README/1772745145572.png)

$\lambda = 1.0$

![1772745211759](image/README/1772745211759.png)
